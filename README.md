...
- **课表设置面板**（v3.6）—— 课表页右上角 ⋮ 打开：周数滑杆拖动即实时翻周、
  课表卡片点一下切换、底部「调课 / 添加课表」两个动作。
- **本地调课**（v3.6）—— 老师临时挪课 / 换教室 / 某周停课时，选课程改到新时间即可；
  可只调一周，也可「之后每周」。被调走的课在原位置留虚线影子、新位置加橙色描边 +
  「调」角标，表格下方再列一份清单，改动一目了然。只改本机显示，教务系统不受影响，
  重新导入后记录依然有效，可逐条撤销。
